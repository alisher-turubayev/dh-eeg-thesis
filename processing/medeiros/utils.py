"""
Utility functions for automated data processing of the BASE Mental Effort 
...
def assemble_windows_array(s: pd.Series, w_size: int, ovlp: int) -> list:
    """
...
def extract_features(df_unprocessed: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for chan_name in df_unprocessed.columns:
        ...
        for window in chan_windows:
            ...
            f_window['f_norm_mean'] = np.mean(
                (window - np.min(window)) / (np.max(window) - np.min(window))
            )
            ...
            f_window[
                'f_low_gamma_rel_power'] = f_window['f_low_gamma_power'] / f_window['f_total_power']
            f_window['f_mid_gamma_power'] = simpson(psd[idx_mid_gamma], dx = freqs_res)
            f_window[
                'f_mid_gamma_rel_power'] = f_window['f_mid_gamma_power'] / f_window['f_total_power']
            f_window['f_hi_gamma_power'] = simpson(psd[idx_hi_gamma], dx = freqs_res)
            f_window[
                'f_hi_gamma_rel_power'] = f_window['f_hi_gamma_power'] / f_window['f_total_power']
            ...
            # Brain indicies
            f_window['f_brain_index1'] = f_window['f_beta_power'] / (
                f_window['f_theta_power'] + f_window['f_alpha_power']
                )
            f_window['f_brain_index2'] = f_window['f_theta_power'] / (
                f_window['f_beta_power'] + f_window['f_alpha_power']
                )
            # Finding what frequency corresponds to maximum alpha frequency
            f_window['f_peak_alpha'] = np.argmax(psd[idx_alpha])

            df_chan_uf = pd.concat(
                [df_chan_uf, pd.DataFrame(f_window, index = [0])], ignore_index = True)
    ...
    for _, row in df_uf.iterrows():
        ...
        for pair in ELECTRODE_PAIRS:
            ...
            f_pair[x + '_' +  y + '_f_da_lg'] = row[
                x + '_f_low_gamma_power'] - row[y + '_f_low_gamma_power']
            f_pair[x + '_' +  y + '_f_da_mg'] = row[
                x + '_f_mid_gamma_power'] - row[y + '_f_mid_gamma_power']
            f_pair[x + '_' +  y + '_f_da_hg'] = row[
                x + '_f_hi_gamma_power'] - row[y + '_f_hi_gamma_power']
            ...
            f_pair[x + '_' +  y + '_f_aa_lg'] = row[
                x + '_f_low_gamma_power'] / row[y + '_f_low_gamma_power']
            f_pair[x + '_' +  y + '_f_aa_mg'] = row[
                x + '_f_mid_gamma_power'] / row[y + '_f_mid_gamma_power']
            f_pair[x + '_' +  y + '_f_aa_hg'] = row[
                x + '_f_hi_gamma_power'] / row[y + '_f_hi_gamma_power']
